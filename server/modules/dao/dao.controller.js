...
const load = async (req, res) => {
    const { _id } = req.user;
    try {
        const dao = await DAO.find({ deletedAt: null, 'members.member': { $in: [_id] } }).populate({ path: 'safe safes sbt members.member projects tasks', populate: { path: 'owners members members.member tasks transactions project metadata' } }).sort({ createdAt: -1 })
